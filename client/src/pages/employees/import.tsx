...
import { X, Upload, AlertCircle, FileSpreadsheet, Check, Download, Users, RefreshCw } from 'lucide-react';
...
const IMPORT_STEPS = [
  { id: 1, name: 'Upload', completed: false, current: true },
  { id: 2, name: 'Review', completed: false, current: false },
  { id: 3, name: 'Complete', completed: false, current: false },
];
...
const EmployeeImportPage: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      const savedData = sessionStorage.getItem(SESSION_STORAGE_KEYS.PROCESSED_DATA);
      const savedStats = sessionStorage.getItem(SESSION_STORAGE_KEYS.IMPORT_STATS);
  ...
  useEffect(() => {
    try {
      ...
      if (processedData) {
        sessionStorage.setItem(SESSION_STORAGE_KEYS.PROCESSED_DATA, JSON.stringify(processedData));
      }
      
      if (importStats) {
        sessionStorage.setItem(SESSION_STORAGE_KEYS.IMPORT_STATS, JSON.stringify(importStats));
      }
    } catch (error) {
    ...
    try {
      ...
      const dataWithIds = {
        ...
        extractedData: result.extractedData.map((row: Record<string, any>) => ({
          ...row,
          id: row.id || `row-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`
        }))
      };
  ...
  const handleConfirmImport = () => {
    ...
    const nonExcludedRows = processedData.extractedData.filter(row => !row.excluded);
  ...
  const handleDownloadProcessedData = () => {
    ...
    try {
      downloadTransformedData(processedData.extractedData, processedData.fileName);
  ...
  const handleDeleteRow = (rowId: string) => {
    ...
    setProcessedData({
      ...processedData,
      extractedData: processedData.extractedData.filter(row => row.id !== rowId)
    });
  ...
  const handleBulkDelete = () => {
    ...
    setProcessedData({
      ...processedData,
      extractedData: processedData.extractedData.filter(row => !filteredIds.includes(row.id))
    });
          ...
          <div className="space-y-8">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Upload an Excel or CSV file containing employee payroll data for processing and import.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <FileUploadZone 
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Review, edit, and validate the extracted employee data before importing.
                </CardDescription>
              </CardHeader>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader className="text-center pb-6">
                ...
                <CardTitle className="text-2xl font-bold">Import Successful</CardTitle>
                <CardDescription className="text-base">
                  {importStats ? `Successfully imported ${importStats.successfulImports} employees.` : 'Your data has been imported successfully.'}
                </CardDescription>
              </CardHeader>
  ...
  return (
    <div className="container mx-auto py-6 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Import Payroll Data</h1>
          <p className="text-muted-foreground">
            Upload, validate and import employee payroll data in a few simple steps.
          </p>
        </div>
      </div>
      ...
      <Dialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to cancel the import process? All uploaded data and progress will be lost.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {processedData && (
                <>
                  You are about to import {processedData.extractedData.filter(r => !r.excluded).length} employee records. 
      ...
      <Dialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete {getFilteredData().length} records? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
