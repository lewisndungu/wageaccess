  ...
  return (
    <div className="flex min-h-screen">
      ...
      >
          ...
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {user?.role === "hr" ? "HR Manager" : user?.role || "User"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 dark:border-gray-800">
          <div className="flex flex-col space-y-2">
            <a
              href="#"
              className="text-xs text-gray-500 dark:text-gray-400 hover:text-primary dark:hover:text-secondary"
            >
              Help & Support
            </a>
            <p className="text-xs text-gray-500 dark:text-gray-400">
      ...
      <div className="fixed bottom-4 right-4 z-50 lg:hidden">
        <Button onClick={toggleSidebar} size="icon" className="rounded-full">
          <MenuIcon className="h-5 w-5" />
        </Button>
      </div>

      {/* Main content */}
      <div className="flex-1 grid grid-rows-[auto_1fr_auto] lg:ml-64">
        {/* Header */}
        ...
        {/* Main content */}
        <main className="p-4 lg:p-6 animate-fade-in bg-gray-100/50 dark:bg-gray-900">
          {/* Global system context header - only shown on certain pages */}
          {(location.pathname.startsWith("/attendance") ||
            location.pathname.startsWith("/payroll") ||
            location.pathname.startsWith("/ewa")) && <GlobalHeader />}
          <AppErrorBoundary>{children}</AppErrorBoundary>
        </main>

        {/* Footer */}
        <footer className="mt-auto h-16 py-6 px-4 lg:px-6 border-t border-gray-200 dark:border-gray-800">
          <div className="container mx-auto flex flex-col md:flex-row items-center justify-between">
            <div className="flex items-center mb-4 md:mb-0">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                  © 2025 Jahazii. All rights reserved.
                </p>
              </div>
            </div>
            <div className="flex space-x-4">
              <a
                href="#"
                className="text-sm text-gray-600 dark:text-gray-400 hover:text-primary dark:hover:text-secondary"
              >
                Privacy Policy
              </a>
              <a
